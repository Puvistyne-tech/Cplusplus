...
//c'est moins couteux de passer une compie de string
int count_a_occurrences(const string& s);
...
//float est moins couteux de passer une réference
void update_loop(const float dt, string& errors_out);
...
//    -> negative_count is 2
bool are_all_positives(const vector<int>& values, int*& negative_indices_out, size_t& negative_count_out);

// Concatenate 'str1' and 'str2' and return the result.
// The input parameters are not modified by the function.
string concatenate(const char* str1, const char* str2);
